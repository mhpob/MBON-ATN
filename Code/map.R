...
md_coast <- read_sf(
  'data and imports/mapping/mid-atlantic/matl_states_land.shp'
) %>%
  filter(!is.na(STATE_NAME)) %>%
  st_transform(4326)

atl_coast <- read_sf(
  'data and imports/mapping/natural earth/ne_10m_coastline.shp'
) %>%
  st_transform(4326)
...
lanes <- read_sf(
  'data and imports/mapping/shippinglanes/shippinglanes.shp',
...
main <-
  ...
  geom_sf(
    data = sites,
    aes(
      alpha = !is.na(proposed_data_type),
  ...
  coord_sf(xlim = c(-75.15, -74.34), ylim = c(38.14, 38.46)) +
...
inset <- ggplotGrob(
  ggplot() +
    ...
    coord_sf(xlim = c(-77.5, -71), ylim = c(35.5, 41)) +
    annotate(
      'rect',
      xmin = -75.15,
      xmax = -74.34,
